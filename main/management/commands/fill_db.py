...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for service_data in services_data:
            category = next(
                (c for c in categories if c.name == service_data["category"]), None
            )
            if category:
        ...
        # Создаем группу "доктор"
        doctor_group, created = Group.objects.get_or_create(name="доктор")
        if created:
            self.stdout.write(self.style.SUCCESS('Создана группа "доктор"'))

        # Получаем все услуги
        all_services = Service.objects.all()

        # Для каждой услуги создаем двух врачей и добавляем их в группу
        for service in all_services:
            for i in range(1, 3):
                username = f"doctor_{service.id}_{i}"
                doctor, created = user_model.objects.get_or_create(
                    username=username,
                    defaults={
                        "first_name": f"Врач {service.name.split()[0]}",
                        "last_name": f"{i}",
                        "is_staff": True,
                        "is_superuser": False,
                        "email": f"{username}@medcheck.ru",
                    },
                )
                if created:
                    doctor.set_password("doctor123")
                    doctor.save()
                    # Добавляем врача в группу "доктор"
                    doctor.groups.add(doctor_group)
                    # Добавляем врача в список врачей услуги
                    service.doctors.add(doctor)
                    self.stdout.write(
                        f"Создан врач {username} для услуги {service.name}"
                    )
